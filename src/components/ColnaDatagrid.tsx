...
export const ColnaDatagrid: React.FC<ColnaDatagridProps> = ({
  ...
  const monthActiveCount = monthRecords.filter(r => !r.zaplatena && !r.isClosed).length;
  const monthCompletedCount = monthRecords.filter(r => r.zaplatena || r.isClosed).length;
    ...
    <div className="bg-white border border-slate-200 rounded-xl shadow-xs overflow-hidden my-4">
      ...
      <div className="overflow-x-auto w-full">
        <table className="w-full text-left text-xs border-collapse">
          <thead>
            <tr className="bg-slate-200 text-black uppercase font-extrabold tracking-wider border-b border-slate-300 select-none text-[16px] leading-tight font-sans">
              ...
              <th rowSpan={2} className="p-2 min-w-[180px] border-r border-slate-300 text-black">
                ZÁKAZNÍK
              </th>
              <th rowSpan={2} className="p-2 w-8 text-center border-r border-slate-300 text-black" title="Nové (NEW)">
                NEW
              </th>
              <th rowSpan={2} className="p-2 w-7 text-center border-r border-slate-300 text-black" title="Upozornenie">
                🔔
              </th>
              <th rowSpan={2} className="p-2 w-7 text-center border-r border-slate-300 text-black" title="Dôležité">
                ⚠️
              </th>
              <th rowSpan={2} className="p-2 min-w-[90px] border-r border-slate-300 text-black">
                DÁTUM
              </th>
              <th rowSpan={2} className="p-2 min-w-[110px] border-r border-slate-300 text-black">
                ŠPZ 🚛
              </th>
              <th rowSpan={2} className="p-2 min-w-[100px] border-r border-slate-300 font-sans text-black">
                REF. NA FAKTÚRU
              </th>
              ...
              <th colSpan={2} className="p-1.5 text-center border-r border-b border-slate-300 text-black bg-emerald-100 font-bold text-[16px] leading-[20px] font-sans">
                EU ➔ UK
              </th>
              ...
              <th rowSpan={2} className="p-2 min-w-[70px] text-right border-r border-slate-300 text-black">
                FA NA KLIENTA
              </th>
              <th rowSpan={2} className="p-2 min-w-[110px] border-r border-slate-300 text-black">
                POZNÁMKA
              </th>
              <th rowSpan={2} className="p-2 min-w-[60px] text-right border-r border-slate-300 text-black font-extrabold">
                ZISK (€)
              </th>
              ...
              <th rowSpan={2} className="p-2 w-14 text-center text-black">
                ÚHRADA
              </th>
            </tr>
              ...
              <tr>
                <td colSpan={21} className="p-10 text-center font-sans">
                  <div className="flex flex-col items-center justify-center gap-2 max-w-md mx-auto py-4">
                    ...
                    <button
                      onClick={onAddRecord}
                      className="bg-blue-600 hover:bg-blue-700 text-white font-bold px-4 py-2 rounded-lg text-xs shadow-xs flex items-center gap-2 cursor-pointer transition-colors"
                    >
                      <Plus className="w-4 h-4" />
                      Pridať nový záznam pre {currentMonthYear}
                    </button>
                  </div>
                </td>
              </tr>
              ...
              paginatedRecords.map((r, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="p-2 text-center border-r border-slate-200">
                      {r.isNew && (
                        <span className="inline-flex items-center justify-center bg-emerald-100 text-emerald-800 border border-emerald-300 w-4 h-4 rounded text-[10px]" title="Nové">
                    ...
                    <td className="p-2 text-center border-r border-slate-200">
                      {r.bell && (
                        <span className="inline-flex items-center justify-center bg-amber-100 text-amber-800 border border-amber-300 w-4 h-4 rounded text-[10px]" title="Upozornenie">
                          <Check className="w-3 h-3 stroke-[3]" />
                        </span>
                      )}
                    </td>
                    ...
                    <td className="p-2 text-center border-r border-slate-200">
                      {r.alert && (
                        <span className="text-red-600 font-bold" title="Dôležité / Súrne">
                    ...
                    {/* Datum Colnice */}
                    <td className="p-2 text-slate-700 border-r border-slate-200 font-mono text-[12px]">
                    ...
                    {/* INT. POZNAMKA */}
                    <td className="p-2 text-slate-500 font-sans text-[11px] border-r border-slate-200 max-w-[150px] truncate" title={r.intPoznamka}>
                      {r.intPoznamka}
                    </td>
                    ...
                    <td className="p-2 text-center border-r border-slate-200">
                      <input
                        type="checkbox"
                        checked={r.zaplatena}
                        onChange={(e) => onTogglePaid(r.id, e.target.checked)}
                        className="rounded text-emerald-600 focus:ring-0 w-3.5 h-3.5 cursor-pointer border-slate-300"
                        title={r.zaplatena ? 'Zaplatené' : 'Nezaplatené (Kliknite pre zmenu)'}
                      />
                    </td>
                  </tr>
                );
          ...
          {/* Totals Row */}
          <tfoot>
            <tr className="bg-slate-200 text-slate-900 font-bold font-mono text-xs border-t-2 border-slate-300">
              <td colSpan={13} className="p-2.5 text-right uppercase tracking-wider font-sans border-r border-slate-300 text-[14px]">
                Spolu:
              </td>
