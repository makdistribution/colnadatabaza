    ...
    <div className="bg-white border border-slate-200 rounded-xl shadow-xs overflow-hidden my-4">
      ...
      <div className="overflow-x-auto w-full">
        <table className="w-full text-left text-xs border-collapse">
          <thead>
            <tr className="bg-slate-50 text-slate-600 uppercase font-bold tracking-wider border-b border-slate-200 text-[11px]">
              <th className="p-2 w-14 text-center border-r border-slate-200 text-[16px]">Akcia</th>
              <th className="p-2 w-12 text-center border-r border-slate-200 text-[16px]">P.Č.</th>
              <th className="p-2 border-r border-slate-200 text-blue-700 font-bold">NÁZOV FIRMY</th>
              <th className="p-2 border-r border-slate-200">REGISTROVANÁ ADRESA</th>
              <th className="p-2 w-14 border-r border-slate-200 text-center">KRAJINA</th>
              <th className="p-2 border-r border-slate-200 font-mono">IČO</th>
              <th className="p-2 border-r border-slate-200 font-mono">DIČ</th>
              <th className="p-2 border-r border-slate-200 font-mono">IČ DPH</th>
              <th className="p-2 border-r border-slate-200">TELEFÓNNE ČÍSLO</th>
              <th className="p-2 border-r border-slate-200">E-MAIL</th>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((r, idx) => {
                ...
                <tr key={r.id} className={`transition-colors hover:bg-blue-50/50 ${idx % 2 === 1 ? 'bg-slate-50/40' : 'bg-white'}`}>
                  ...
                  <td className="p-2 font-mono text-slate-700 font-bold border-r border-slate-100 text-center">{pcFormatted}</td>
                  <td className="p-2 font-bold text-slate-900 border-r border-slate-100">{r.nazovFirmy}</td>
                  <td className="p-2 text-slate-700 border-r border-slate-100 flex items-center gap-1.5">
                  ...
                  <td className="p-2 font-mono text-slate-700 border-r border-slate-100">{r.ico}</td>
                  <td className="p-2 font-mono text-slate-700 border-r border-slate-100">{r.dic}</td>
                  <td className="p-2 font-mono text-slate-700 border-r border-slate-100">{r.icDph}</td>
                  ...
                  <td className="p-2 text-slate-700 border-r border-slate-100">
                    {r.email && (
                      <a href={`mailto:${r.email}`} className="text-blue-600 underline hover:text-blue-800 flex items-center gap-1.5 font-semibold">
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-xl w-full max-w-lg p-6 text-slate-900 space-y-4">
            ...
            <form onSubmit={handleSave} className="space-y-3.5 text-xs">
              ...
              <div className="flex justify-end gap-2.5 pt-4 border-t border-slate-100">
                ...
                >
                  <Save className="w-4 h-4" /> Uložiť firmu
                </button>
              </div>
            </form>
          </div>
