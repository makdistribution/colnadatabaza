...
export const ReportyView: React.FC<ReportyViewProps> = ({ records, year, onYearChange }) => {
          ...
          <div className="bg-slate-50 border border-slate-200 p-4 rounded-xl flex items-center justify-between">
            <div>
              ...
              <span className="text-xl font-black text-emerald-600 font-mono">
                {totalYearProfit.toFixed(2).replace('.', ',')} €
              </span>
            </div>
          ...
          <div className="bg-slate-50 border border-slate-200 p-4 rounded-xl flex items-center justify-between">
            <div>
              ...
              <span className="text-xl font-black text-blue-600 font-mono">
                {totalYearRevenue.toFixed(2).replace('.', ',')} €
              </span>
            </div>
            <DollarSign className="w-7 h-7 text-blue-500/30" />
          </div>
            ...
            return (
              <div key={group.monthIndex} className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-xs">
                ...
                <button
                  ...
                  className="w-full bg-slate-50 hover:bg-slate-100/80 px-5 py-3 border-b border-slate-200 flex items-center justify-between text-xs transition-colors cursor-pointer text-left"
                >
                  ...
                  <div className="flex items-center gap-3">
                    <div className="bg-emerald-50 border border-emerald-200 text-emerald-800 font-bold px-3 py-1 rounded-md font-mono text-xs flex items-center gap-1.5">
                      <span className="text-emerald-600">Zisk:</span>
                      <span>{group.totalProfit.toFixed(2).replace('.', ',')} €</span>
                    </div>
                  </div>
                </button>
                ...
                {!isCollapsed && (
                  <div className="overflow-x-auto">
                    <table className="w-full text-left text-xs border-collapse">
                      <thead>
                        <tr className="bg-slate-50 text-slate-600 uppercase font-bold tracking-wider border-b border-slate-200 text-[10px]">
                          <th className="p-2.5 min-w-[120px] border-r border-slate-200">ZÁKAZNÍK</th>
                          <th className="p-2.5 min-w-[100px] border-r border-slate-200">DÁTUM COLNICE</th>
                          <th className="p-2.5 min-w-[120px] border-r border-slate-200">ŠPZ 🚛</th>
                          <th className="p-2.5 min-w-[120px] border-r border-slate-200">REF. NA FA.</th>
                          <th className="p-2.5 min-w-[120px] border-r border-slate-200 text-blue-700">UK ➔ EU</th>
                          <th className="p-2.5 min-w-[120px] border-r border-slate-200 text-emerald-700">EU ➔ UK</th>
                          <th className="p-2.5 text-right border-r border-slate-200">NÁKLADY UK AGENT</th>
                          <th className="p-2.5 text-right border-r border-slate-200">NÁKLADY EU AGENT</th>
                          <th className="p-2.5 text-right border-r border-slate-200 text-blue-700">FA. ➔ KLIENT</th>
                          <th className="p-2.5 text-right border-r border-slate-200 text-emerald-700 font-extrabold">ZISK</th>
                          <th className="p-2.5 min-w-[90px] border-r border-slate-200">ČÍSLO FA.</th>
                          <th className="p-2.5 min-w-[90px] border-r border-slate-200">SPLATNÁ</th>
                          <th className="p-2.5 text-center w-16">ZAPLATENÁ</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100 font-mono text-[11px] text-slate-800">
                        {group.records.map((r, idx) => (
                          <tr key={r.id} className={`transition-colors hover:bg-blue-50/50 ${idx % 2 === 1 ? 'bg-slate-50/40' : 'bg-white'}`}>
                            <td className="p-2 font-sans font-bold text-slate-900 border-r border-slate-100">
                              {r.zakaznik}
                            </td>
                            <td className="p-2 text-slate-700 border-r border-slate-100">
                            ...
                            <td className="p-2 text-blue-700 border-r border-slate-100 font-sans text-[11px] font-medium">
                              {r.ukToEu}
                            </td>
                            <td className="p-2 text-emerald-700 border-r border-slate-100 font-sans text-[11px] font-medium">
                              {r.euToUk}
                            </td>
                            <td className="p-2 text-right border-r border-slate-100 text-slate-700">
                              {r.faOdUkAgent ? `${r.faOdUkAgent.toFixed(2).replace('.', ',')}` : '0,00'}
                            </td>
                            <td className="p-2 text-right border-r border-slate-100 text-slate-700">
                              {r.faOdEuAgent ? `${r.faOdEuAgent.toFixed(2).replace('.', ',')}` : '0,00'}
                            </td>
                            <td className="p-2 text-right border-r border-slate-100 font-bold text-blue-800 bg-blue-50/30">
                              {r.faKlient ? `${r.faKlient.toFixed(2).replace('.', ',')}` : '0,00'}
                            </td>
                            <td className="p-2 text-right border-r border-slate-100 font-extrabold text-emerald-700 bg-emerald-50/50">
                              {r.zisk ? `${r.zisk.toFixed(2).replace('.', ',')}` : '0,00'}
                            </td>
                            <td className="p-2 text-slate-700 border-r border-slate-100 font-medium">
                              {r.cisloFa}
                            </td>
                            <td className="p-2 text-slate-700 border-r border-slate-100">
