...
export const appApi = {
  ...
  getEmailSignature: () =>
    request<{ html: string }>('/api/app', {
      method: 'POST',
      body: JSON.stringify({ action: 'getEmailSignature' }),
    }),
  saveEmailSignature: (signatureHtml: string) =>
    request<{ html: string }>('/api/app', {
      method: 'POST',
      body: JSON.stringify({ action: 'saveEmailSignature', signatureHtml }),
    }),
  ...
  searchVatEori: (payload: {
    ...
    request<{
      ...
      method: 'POST',
      body: JSON.stringify({ action: 'searchVatEori', ...payload }),
    }),
  getReportPayouts: () =>
    request<{ keys: string[] }>('/api/app', {
      method: 'POST',
      body: JSON.stringify({ action: 'getReportPayouts' }),
    }),
  markReportPayout: (monthKey: string, pin: string) =>
    request<{ ok: boolean; error?: string; keys: string[] }>('/api/app', {
      method: 'POST',
      body: JSON.stringify({ action: 'markReportPayout', monthKey, pin }),
    }),
};
